...
class Plot2D:
    ...
    def plot_scalar_field(self, ax, section_name=None, cell_number=None, sn=0, direction='y',
        ...
        ax.contourf(image, cmap='autumn', extent=extent_val, zorder=8, **kwargs)
    ...
    def plot_data(self, ax, section_name=None, cell_number=None, direction='y',
        ...
        if section_name is not None:
            if section_name == 'topography':
                ...
                points[['X']] = np.atleast_2d(np.linalg.norm(cartesian_point, axis=1)).T
                orientations[['X']] = np.atleast_2d(np.linalg.norm(cartesian_ori, axis=1)).T
                #### modify gempy to my
